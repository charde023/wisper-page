...
| P2 — 맥(M4 Max) 로컬 전사 이식 | `faster-whisper(CUDA)` → `mlx-whisper`(Metal GPU), `.ps1` 3종 → 파이썬 이식 | 🔶 제안(구현 대기) — 설계: `design/2026-07-02_맥-로컬-전사-파이프라인-기획서.md` |
| P3 — 학습노트 자동화(무인 파이프라인) | 감시→전사→코덱스 프록시 교정/노트화→발행→알림, launchd 야간 스케줄 | 🔶 제안(결정 확정, 구현 대기) — 설계: `design/2026-07-02_학습노트-자동화-파이프라인-기획서.md` |

- ✅ = 운영 중(회귀 시 즉시 보수). 🔶 = 설계 완료·구현 미착수(Align/Spec까지 끝, Plan/Build 대기).
